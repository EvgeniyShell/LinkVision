 ...
 *  - autoungrabify — узлы нельзя перетаскивать (для существующих и будущих);
 *  - events:'no' на всех элементах — указатель проходит «сквозь» устройства,
 *    группы и фигуры, поэтому карту можно панорамировать перетаскиванием НАД
 *    любым элементом (в т.ч. над большой фигурой-«столом»), а не только по
 *    пустому полю.
 */
function applyDragLockToCanvas(isLocked) {
    ...
    if (isLocked) {
        cy.elements().style('events', 'no');
    } else {
        cy.elements().removeStyle('events');
    }
}
